...
FHeap.prototype._checkIfMin = function(item) {
    if (this.greaterThan(this.min.data, item.data)) {
...
FHeap.prototype._makeLink = function(item, prevItem) {
    ...
    var firstChild = null;
    var lastChild = null;
    if (this.greaterThan(item.data,prevItem.data)) {
        lesserItem = prevItem;
        greaterItem = item;
    }
...
FHeap.prototype._consolidateHeap = function(rootlist) {
    ...
    for (var i = 0; i < fillArray.length; i++) {
        ...
        if (this.min == null) {
            this.min = item;
        } else if (this.greaterThan(this.min.data,item.data)) {
            this.min = item;
        }
